...
def main():
    os.system('clear')
    try:
        with open(f'./querys/querys.txt', 'r', encoding = 'utf-8') as file:
            os.system('clear')
            extract_day = datetime.date.today() - datetime.timedelta(days=6)
            day = int(extract_day.strftime('%d'))
            month = int(extract_day.strftime('%m'))
            file_name = ''
            if month < 10:
                file_name = f'extracted_tweets_0{month}22.csv'
            else:
                file_name = f'extracted_tweets_{month}22.csv'

            print(day, month, file_name)
            os.system('clear')
